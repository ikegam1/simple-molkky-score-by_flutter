...
class _SetupScreenState extends State<SetupScreen> {
  ...
  Future<void> _signInWithGoogle() async {
    try {
      ...
      if (mounted) {
        _showError(
          'Googleログインに失敗しました (${e.code}: ${e.message ?? "no message"})',
        );
      }
    } catch (e) {
  ...
  Future<void> _signInWithApple() async {
    try {
      ...
      // Apple Sign-In は nonce ベース認証のため、linkWithCredential が
      // credential-already-in-use で失敗すると nonce が消費され、
      // 同じ credential で signInWithCredential しても
      // "Duplicate credential received" になる。
      // そのため linkWithCredential → fallback パターンは使わず、
      // 常に signInWithCredential を使い、データマージは手動で行う。

      // 1. まだ匿名 UID として認証中のうちにデータを読み取る
      List<Map<String, dynamic>> oldData = <Map<String, dynamic>>[];
      try {
        oldData = await _fetchAllScoreData(oldUid);
      } catch (fetchErr) {
        debugPrint('Apple: fetchAllScoreData failed (継続): $fetchErr');
      }

      // 2. Apple credential でサインイン (既存ユーザなら復帰、新規なら作成)
      final result = await FirebaseAuth.instance.signInWithCredential(
        credential,
      );
      final newUid = result.user!.uid;

      // 3. UID が変わった場合、匿名 UID のデータを新 UID にマージ
      if (oldUid != newUid && oldData.isNotEmpty) {
        try {
          await _writeScoreDataAsNewUid(oldData, newUid);
        } catch (writeErr) {
          debugPrint('Apple: writeScoreDataAsNewUid failed (継続): $writeErr');
        }
      }
      setState(() {
        _firebaseUid = newUid;
      ...
      if (mounted)
        _showError(
          'Appleログインに失敗しました (${e.code}: ${e.message ?? "no message"})',
        );
    } catch (e) {
  ...
  Future<void> _deleteAccount() async {
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        if (mounted) {
          _showError('セキュリティ上、最近のサインインが必要です。一旦サインアウトして再サインインしてから削除してください');
        }
      } else {
...
class _GameScreenState extends State<GameScreen>
  ...
  // アノテーションピッカーOverlay
  OverlayEntry? _annotationPickerEntry;
  final ValueNotifier<int> _pickerAnnotation = ValueNotifier(0);
  // セット間Undo用スナップショット
  ...
  // ドラッグ方向からアノテーション値を決定
  // 1=◯(down) / 2=□(up) / 3=寄せ(left) / 4=飛ばし(right)
  int _annotationFromOffset(Offset offset) {
    const threshold = 22.0;
    if (offset.distance < threshold) return 0;
    if (offset.dy.abs() >= offset.dx.abs()) {
      return offset.dy < 0 ? 2 : 1;
    } else {
      return offset.dx < 0 ? 3 : 4;
    }
  }
  ...
  void _updateLastAnnotation(String playerId, int annotation) {
    if (turnInProgressScores.containsKey(playerId)) {
      if (annotation == 0) {
        _turnAnnotations.remove(playerId);
      } else {
        _turnAnnotations[playerId] = annotation;
      }
    } else if (widget.match.currentSetRecord.turns.isNotEmpty) {
      widget.match.currentSetRecord.turns.last.scoreAnnotations[playerId] =
          annotation;
    }
  }
  ...
  void _upgradeLastAnnotation(String playerId) =>
      _updateLastAnnotation(playerId, 1);
  ...
  static Widget _annotatedScoreWidget(
    ...
    final c = style.color ?? Colors.black;
    final fs = style.fontSize ?? 14;
    final smallStyle = style.copyWith(fontSize: fs * 0.62, color: c);
  ...
  List<Widget> _jaSections(L10n t) => [
    ...
    const _HelpSection(
      ...
      items: [
        ...
        'iPhone・iPad はテンキーなどの外付けキーボードを繋いでいる間、画面のキーボードが出ません（iOS の仕様）。iPhone なら「設定」→「アクセシビリティ」→「タッチ」→「背面タップ」で「ダブルタップ」に「AssistiveTouch」を割り当てておくと、背面を 2 回叩くだけでキーボードを呼び出せます（もう一度叩くと戻ります）。背面タップは iPhone 専用なので、iPad では名前を先に入力しておくか Bluetooth を一度切ってください',
  ...
  List<Widget> _enSections(L10n t) => [
    ...
    const _HelpSection(
      ...
      items: [
        ...
        'On iPhone/iPad the on-screen keyboard stays hidden while an external keyboard (including a numpad) is connected. On iPhone, assign AssistiveTouch to Back Tap (Settings > Accessibility > Touch > Back Tap > Double Tap) and you can bring the keyboard up by tapping the back of the phone twice. Back Tap is iPhone-only, so on iPad enter player names before connecting the keyboard, or turn Bluetooth off briefly',
...
// ─── ピンボタン（押下ビジュアル / 長押しフリックでアノテーション選択）──────────
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: _pressed ? Colors.blue : Colors.grey[300]!,
            width: _pressed ? 1.8 : 1.0,
          ),
          borderRadius: BorderRadius.circular(8),
        ),
        alignment: Alignment.center,
        child: Text(
          ...
          style: TextStyle(
            fontSize: widget.fontSize,
            fontWeight: FontWeight.bold,
            color: _pressed ? Colors.blue[800] : Colors.black,
          ),
        ),
      ),
    );
...
// ─── アノテーションフリックピッカー ──────────────────────────────────────────
// 長押し / 再タップ時にオーバーレイ表示。上下左右にスライドで選択。
class _AnnotationPicker extends StatelessWidget {
  const _AnnotationPicker({
    required this.center,
    required this.pickerAnnotation,
    required this.onDismiss,
  });

  final Offset center;
  final ValueNotifier<int> pickerAnnotation;
    ...
    return GestureDetector(
      ...
      child: Stack(
        children: [
          // Up = □囲み
          _positioned(
            left: center.dx - _itemSize / 2,
            top: center.dy - _itemSize * 2 - _gap,
            annotation: 2,
            label: '□囲み',
            icon: '↑',
          ),
          ...
          // Left = 寄せ成功
          _positioned(
            left: center.dx - _itemSize * 2 - _gap,
            top: center.dy - _itemSize / 2,
            annotation: 3,
            label: '寄せ',
            icon: '←',
          ),
  ...
  Widget _positioned({
    ...
    return Positioned(
      ...
      child: ValueListenableBuilder<int>(
        valueListenable: pickerAnnotation,
        builder: (_, selected, __) {
          final isSelected = selected == annotation;
          return GestureDetector(
            onTap: () {
              pickerAnnotation.value = annotation;
              onDismiss();
            },
            child: AnimatedContainer(
