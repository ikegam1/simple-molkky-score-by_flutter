      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            const Text('v1.10.10', style: TextStyle(color: Colors.grey, fontSize: 12)),
          ],
        ),
      ),
    );
...
class _GameScreenState extends State<GameScreen> {
  ...
  int? _parseVoiceScore(String text) {
    ...
    //    発話長ガード: 短い発話のみ照合し、背景会話による誤検知を防ぐ
    //    （発話テキスト長 <= 単語長 + 3文字 の場合のみマッチ）
    ...
    for (final (word, score) in jaHomophones) {
      if (ja.length <= word.length + 3 && ja.contains(word)) return score;
    }

    // 4. ミス判定: フォルト / 0点 / 0手 / 零点 / 霊点
